$psakeChocoDir = Split-Path $MyInvocation.MyCommand.Path -Parent
...
task Choco:GenerateNuspec {
    if (-not($packageDestination)) {
        throw "Package destination path not configured."
    }
    ...
    if ($packageTitle) {
        $nuspec['Title'] = $packageTitle
    }

    if ($packageOwners) {
        $nuspec['Owners'] = $packageOwners
    }

    if ($projectLicenseUrl) {
        $nuspec['LicenseUrl'] = $projectLicenseUrl
    } elseif ($projectLicense) {
        $ossLicenses = @('Apache-2.0', 'BSD-3-Clause', 'BSD-2-Clause', 'gpl-license',
                        'lgpl-license', 'MIT', 'MPL-2.0', 'CDDL-1.0', 'EPL-1.0')
        if ($ossLicenses -contains $projectLicense) {
            $nuspec['LicenseUrl'] = "https://opensource.org/licenses/$($projectLicense)"
        } else {
            throw "Unknown license '$($projectLicense)'."
        }
    }
    ...
    if ($changeLogFile -and (Test-Path $changeLogFile)) {
        if ($packageVersion -match '-') {
            $packageIsPrerelease = $true
            $packageVersionNumber = [System.Version]::Parse($packageVersion.Split('-')[0])
        } else {
            $packageIsPrerelease = $false
            $packageVersionNumber = [System.Version]::Parse($packageVersion)
        }
        ...
        Get-Content $changeLogFile | foreach {
            if ($isInVersion) {
                ...
                if (-not($packageIsPrerelease) -and $_.StartsWith("## [$($packageVersionNumber)]")) {
                    $isInVersion = $true
                    $releaseNotes += "## [$($packageVersionNumber)]"
                } elseif ($packageIsPrerelease -and $_.StartsWith("## Unreleased")) {
                    $isInVersion = $true
                    $releaseNotes += "## [$($packageVersionNumber)]"
                }
            }
    ...
    Write-Message "Generating nuspec '$($packageId).nuspec' from template..."
	$nuspecFile = Join-Path $packageDestination "$($packageId).nuspec"
	$nuspecTemplateFile = Join-Path $psakeChocoDir "Templates\ChocolateyPackage.nuspec.tmpl"
	$nuspecContent = Expand-Template -File $nuspecTemplateFile -Binding $nuspec
